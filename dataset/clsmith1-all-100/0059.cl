// --atomics 48 ---atomic_reductions ---fake_divergence ---inter_thread_comm -g 58,55,1 -l 2,1,1
#define int64_t long
...
uint16_t  func_1(struct S2 * p_3);


/* --- FUNCTIONS --- */
/* ------------------------------------------ */
/* 
 * reads : p_3->g_comm_values
 * writes:
 */
uint16_t  func_1(struct S2 * p_3)
{ /* block id: 4 */
    if ((atomic_inc(&p_3->l_atomic_input[29]) == 6))
    { /* block id: 6 */
        uint64_t l_2[3];
        int i;
        for (i = 0; i < 3; i++)
        ...
        int l_2_i0;
        for (l_2_i0 = 0; l_2_i0 < 3; l_2_i0++) {
            result += l_2[l_2_i0];
        }
        atomic_add(&p_3->l_special_values[29], result);
    }
    else
    { /* block id: 8 */
        (1 + 1);
    }
    return p_3->g_comm_values[p_3->tid];
}
...
__kernel void entry(__global ulong *result, __global volatile uint *g_atomic_input, __global volatile uint *g_special_values, __global volatile int *g_atomic_reduction, __global int *sequence_input, __global long *g_comm_values) {
    ...
    struct S2 c_5 = {
        ...
        (safe_add_func_uint32_t_u_u((safe_mul_func_uint32_t_u_u(get_linear_group_id(), 2)), permutations[0][get_linear_local_id()])), // p_3->tid
    ...
    if (!get_linear_local_id())
        for (i = 0; i < 48; i++)
            transparent_crc(p_3->l_special_values[i], "p_3->l_special_values[i]", print_hash_value);
    transparent_crc(p_3->l_comm_values[get_linear_local_id()], "p_3->l_comm_values[get_linear_local_id()]", print_hash_value);
    transparent_crc(p_3->g_comm_values[get_linear_group_id() * 2 + get_linear_local_id()], "p_3->g_comm_values[get_linear_group_id() * 2 + get_linear_local_id()]", print_hash_value);
